...
/**
 * Shipping row — same source FunnelKit's shipping_total_fee uses:
 * WC cart shipping total for the currently chosen method.
 *
...
function wchs_checkout_mini_cart_shipping_display( WC_Cart $cart ): array {
	...
	// Chosen method already applied by update_checkout into the cart totals.
	...
	$chosen = ( function_exists( 'WC' ) && WC()->session )
		? array_filter( array_map( 'strval', (array) WC()->session->get( 'chosen_shipping_methods', [] ) ) )
		: [];

	// Address entered but no method selected yet — don't pretend Free.
	if ( empty( $chosen ) && $shipping_total <= 0.009 ) {
		return [
			'html'     => '—',
			'is_free'  => false,
			'pending'  => true,
	...
	// Paid method (even when free shipping is also unlocked).
	return [
...
function wchs_checkout_mini_cart_after_mutate(): void {
	if ( function_exists( 'WC' ) && WC()->cart ) {
		// Keep chosen method; totals include its WC shipping amount.
...
function wchs_checkout_mini_cart_ajax_refresh(): void {
	wchs_checkout_mini_cart_verify();
	// Do not recalculate shipping here — FunnelKit/WC already ran update_checkout
	// and set chosen_shipping_methods + cart shipping totals. Just re-read them.
	if ( function_exists( 'WC' ) && WC()->cart ) {
		WC()->cart->calculate_totals();
	}
	wchs_checkout_mini_cart_ajax_html();
